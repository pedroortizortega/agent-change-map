...
## Visual redesign — reference-image alignment (post-PR4, pre-PR5)

User-driven pass moving the diagram's design language closer to a reference node-editor-style
screenshot (ComfyUI/shader-graph look): dark dotted-grid background, rounded "card" nodes with a
header/body divider, visible port dots at edge attachment points, and smoothed/rounded orthogonal
edge corners — all while KEEPING the existing custom obstacle-avoiding router in
`webview/edgeGeometry.ts` (not replaced with React Flow's plain bezier/smoothstep edges). Also
folds in a confirmed functional bug fix discovered during this pass (see item 5): live edge
re-routing during an in-progress node drag, documented in design.md §4 but never actually wired.

- [x] 1. Dotted grid background — `webview/index.tsx` renders `<Background variant={BackgroundVariant.Dots}>` (from `@xyflow/react`) as a child of `<ReactFlow>`, styled via `var(--vscode-editorWhitespace-foreground, ...)` (no hardcoded color), `gap={24}`/`size={1}` for a sparse, subtle grid matching the reference image rather than a loud one.
- [x] 2. Card-style nodes — `webview/nodes/AcmEntityNode.tsx` + `webview/styles.css`: `.acm-node-box` fill switched to `--vscode-editorWidget-background` (raised-surface look, same token the relationship popup already uses); new `.acm-node-divider` `<line>` at `HEADER_DIVIDER_Y=26` (matches `edgeGeometry.ts`'s existing `routingPorts` "below title" convention) separating header from body. DECISION (documented in `AcmEntityNode.tsx`'s doc comment): `KIND_STYLE[kind].dasharray` is KEPT as the container/leaf distinguishing convention (real, load-bearing containment info this repo has and the reference image doesn't) — containers get a low-opacity (`opacity: 0.55`) dashed frame with no card background fill, reading as a subtle "grouping" wrapper around their children's own full-strength cards, rather than a second competing raised surface.
- [x] 3. Visible port dots — `webview/edgeGeometry.ts` adds `pathEndpoints(d)` (reads the exact start/end coordinate pair straight off the router's own final `d` string — the `M` point and the final segment's destination point, never a re-derived/estimated position); `webview/graphLayout.ts`'s `buildEdges` calls it once per edge and stores `data.startPoint`/`data.endPoint` on `AcmEdge`; `webview/edges/AcmKindEdge.tsx` renders two `<circle className="acm-edge-port">` at those exact points. Styled uniformly/neutral via CSS (`--vscode-descriptionForeground`), NOT per-kind (this is layout chrome, not kind-differentiated relationship-color language, so it deliberately does not touch `edgeStyleConfig.ts`).
- [x] 4. Rounded/curved edge corners — `webview/edgeGeometry.ts` adds `CORNER_RADIUS` (8px) and `roundedPolylinePath(points, radius)`: shortens each straight leg adjacent to an interior waypoint by `min(radius, halfShorterAdjacentSegment)` and inserts a `Q` (quadratic Bezier, using the original corner as control point) between the shortened ends — same waypoints, same order, same obstacle-avoidance decisions as the sharp-cornered polyline; only the FINAL rendered `d` string changes. Applied only to `edgePathsFor`'s winning coordinated-route string (`webview/edgeGeometry.ts`'s main multi-edge entry point, what `graphLayout.ts` actually renders), not to the single-edge `edgePathFor` fallback (kept byte-identical, preserving its existing exact-string unit tests and the missing-target dashed-stub contract).
  - Test updates: `test/unit/edgeGeometry.test.ts` gained a new `describe("roundedPolylinePath — ...")`/`describe("pathEndpoints")` block (radius clamping, endpoint preservation, custom radius, topology/command-count invariants) — new RED→GREEN coverage, not a rewrite of existing assertions. `test/unit/coordinatedRouting.test.ts`'s one exact-string assertion (`not.toMatch(/[CQ]/)`) was narrowed to `not.toMatch(/C/)` since `Q` corners are now an expected, legitimate part of the coordinated router's output; every other assertion in that file (waypoint extraction via regex, orthogonal-segment/no-crossing/clearance-margin invariants) needed NO changes — the shortened sub-segments the rounding introduces remain axis-aligned subsets of the original straight legs, so those topology-level checks still hold.
  - KNOWN LIMITATION (documented, not fixed in this pass): the rounding curve itself (the `Q` arc, not the extracted waypoint string) is not independently obstacle-clearance-tested pixel-by-pixel; `CORNER_RADIUS` is kept small (8px, under both `LANE_GAP`/`DETOUR_CLEARANCE` = 12px) specifically to bound this risk, but a corner sitting exactly at the `ROUTE_CLEARANCE` (2px) margin from an unrelated box could theoretically have its rounded arc dip fractionally closer to that box than the sharp corner did. No such regression was observed in this repo's own real-analyzer-output fixture (`coordinatedRouting.test.ts`'s "real analyzer output" case still passes).
- [x] 5. BUG FIX (not visual/cosmetic) — live edge re-routing during an in-progress drag, per design.md §4's documented (but never implemented) contract: `webview/index.tsx` had no `onNodesChange` handler at all before this pass, so edges only snapped to correct routing on drop. Added `onNodesChange`: filters for an in-progress (`dragging: true`) `position` change, builds a local `boxes` clone (`boxesForRouting` from `graphLayout.ts`, now exported) patched with the live dragged position plus the same D14 container-descendant cascade `onNodeDragStop` already applies, then re-anchors ONLY the edges touching the dragged node/its cascaded descendants via the cheap single-edge `edgePathFor` (not the full coordinated `edgePathsFor`) — a deliberate O(edges touching this node) live-preview tradeoff over an O(all edges) full re-route on every pointermove; the full coordinated re-route still runs exactly once, on drop, via the pre-existing `onNodeDragStop`/`overrideSeq` path (unchanged, still the only writer of `positionOverrides`). Live overrides are cleared in `onNodeDragStop` so the drop's authoritative full re-route always wins.
  - Test coverage: NOT independently unit/DOM tested in this pass. The pure boxes-clone-and-reroute logic is inline in `onNodesChange` rather than factored into a standalone testable function (a reasonable follow-up), and simulating a genuine "drag in progress, not yet dropped" gesture against React Flow's internal XYDrag pointer-event handling is one of jsdom's known gaps (design.md's own Testing Strategy section already calls out SMIL/live-interaction gaps in jsdom for the related particle-animation case). Verified instead by full static/type/lint/e2e gate + code-level trace of the D14 cascade parity with `onNodeDragStop`. Flagged here explicitly as a manual/visual follow-up rather than skipped silently.
- [x] 6. Final gate — `npm run typecheck`, `npm run lint`, `npm test` (534/534, up from the prior 525/525 baseline: 9 new tests, all in `edgeGeometry.test.ts`'s new `roundedPolylinePath`/`pathEndpoints` coverage; `coordinatedRouting.test.ts` kept its existing 12 tests, one assertion narrowed as noted above), `npm run test:e2e` (exit 0, all 9 scenarios completed for real), `npm run build:webview` (exit 0) all run for real and green.
...
- [x] 5.1 RED — add hover-highlight cases to `webviewDom.test.ts`: hovering a node/edge adds `acm-dim` to non-connected elements and `acm-hot` to the connected subgraph; hover end restores normal styling. Confirm it fails.
- [x] 5.2 GREEN — implement `hoverId` state + `useMemo(highlightNodes/highlightEdges)` via `getIncomers`/`getOutgoers`/`getConnectedEdges` (design §7); add `.acm-dim`/`.acm-hot`/`.acm-particle` CSS rules. Confirm 5.1 passes.
- [x] 5.3 REFACTOR — confirm particle `durationMs` is never mutated by hover (D9); assert `<animateMotion>` is not remounted across a hover toggle. **Found and fixed a real, adjacent bug along the way (not in design.md): naively replacing EVERY node object's reference on each hover (to carry the new `className`) resets React Flow's internal `handleBounds` for every node — `@xyflow/system`'s `parseHandles` only preserves it when the incoming node object either has the SAME reference as before or already carries a `measured` field — which makes every edge touching a reference-changed node briefly report a null position and get unmounted (verified via a DOM-identity assertion). Fixed by stamping `measured: {width, height}` (values `layoutGraph` already computes, never from DOM measurement) onto every node passed to `<ReactFlow>`, restoring D9 for nodes as well as edges.**
- [x] 5.4 Manual measurement — package the `.vsix` before/after migration, record the size delta (proposal success criterion). See apply-progress.md — main: ~726 KB / this branch: ~989 KB (+~257 KB, +36%), both measured the same way (no `.vscodeignore` in this repo on either branch, so both include dev-only files; the delta is still representative since that packaging noise is identical on both sides). Dominant cost: the new bundled `index.js` (~1.1 MB unminified — `minify:false` was a deliberate design choice, D-tableless but stated in design.md §1, to keep the bundle reviewable/diffable).
- [x] 5.5 Manual measurement — probe React Flow at `{nodes:300, edges:600}` with concurrent SMIL particles; if degraded, record as a follow-up (particle capping), not a code change in this PR. **Found a severe, previously-unmeasured algorithmic risk, unrelated to this PR's own hover-highlight code**: `layoutGraph`'s flat-fallback coordinated router (`edgeGeometry.ts`'s `edgePathsFor`, untouched by PR5) scales far worse than linearly — measured `{60,120}`: ~0.9s, `{100,200}`: ~8.8s, `{150,300}`: ~28.8s on this machine; extrapolating, the exact `{300,600}` `OVERSIZED_THRESHOLDS` boundary is on the order of MINUTES of synchronous main-thread compute, which would freeze the webview long before any SMIL particle rendering even becomes relevant. A committed `test/unit/graphLayout.test.ts` probe stays at the fast `{60,120}` size (would otherwise make every `npm test` run hang for minutes); the `{300,600}` numbers were obtained once, out-of-band, via `tsx`, and are recorded here as an explicit follow-up (cap/short-circuit the coordinated crossing-penalty pass above a size threshold, falling back to the already-existing cheap per-edge `edgePathFor` used for live-drag preview, or reduce the router's algorithmic complexity) — not fixed in this PR per the task's own scope boundary.
- [x] 5.6 Final gate: `npm test`, `npm run lint`, `npm run typecheck`, `npm run build` all green before opening PR5.
