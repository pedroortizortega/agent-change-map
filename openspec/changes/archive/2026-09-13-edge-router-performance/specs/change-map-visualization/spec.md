...
The base requirement (see below) — dash/color distinction between `import` and
`call`, no edge for `contains`, obstacle-avoiding detour routing with graceful
degradation (substantially fewer crossings than a direct path, not a guarantee
of zero in general), and ancestor/self-reference suppression — MUST continue to
hold regardless of which routing algorithm produces it. The base requirement's
correctness contract is unchanged by the implementation; only the algorithm may
be replaced.
...
properties tested and verified on each routing algorithm change):
...
drawn. The routing algorithm was implicitly tied to today's specific coordinated
multi-edge algorithm, with no explicit statement that determinism means
reproducibility rather than byte-identical-to-a-specific-algorithm output, and
no explicit spec-level statement of the orthogonal-segment, clearance-margin,
port-lane, self-loop, and outer-lane-fallback properties.)
