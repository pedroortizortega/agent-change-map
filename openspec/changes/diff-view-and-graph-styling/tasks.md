...
- [x] 4.1 RED new case: nested geometry — a child `<g>`'s transform-derived box lies fully inside its parent's rect bounds; three-level nesting (module → class → method) all contained correctly.
- [x] 4.2 RED new case: orphan node — an entity whose `containerId` refers to a container filtered out by `filterGraph` renders as a loose root, no placeholder box.
- [x] 4.3 RED rewrite `graphView.test.ts` L41-47 (`"renders every node and edge…"`): keep as the `data-node-id` stability test; add assertion that `function:pkg.a.f`'s `<g>` is a DOM descendant of `module:pkg.a`'s `<g>`.
- [x] 4.4 GREEN implement `childrenOf` map (containerId normalized to `undefined` when absent from `graph.nodes`), `measure(n)` bottom-up sizing, `place(n, x, y)` top-down absolute placement recording `boxes: Map<nodeId, Rect>`, roots stacked at `x=MARGIN` with `ROOT_GAP`, cycle guard (`visited` set demoting repeat-reached nodes to loose roots). Use named constants `NODE_H=32, HEADER_H=20, PAD_X=12, PAD_Y=10, GAP_Y=8, NODE_MIN_W=200, ROOT_GAP=24, MARGIN=16`.

## Phase 5: Slice B — Kind Encoding and Outline-Only Styling

- [x] 5.1 RED new case: kind encoding — `stroke-width`/`stroke-dasharray`/`rx` match the design table per kind (`package`, `module`, `class`, `function`, `method`); containers strictly thinner than every entity; `class` distinguished from `function`/`method` on both width and `rx`.
- [x] 5.2 RED new case: outline-only — every `<rect class="node-box">` carries `fill="none"`; status class still present.
- [x] 5.3 RED rewrite `graphView.test.ts` L56-65 (`"distinguishes added, removed, and modified…"`): keep `data-change-status`; add `fill="none"` assertion on every rect.
- [x] 5.4 GREEN implement `renderNodeRect` emitting inline `stroke-width`/`stroke-dasharray`/`rx`/`fill="none"` per the kind-encoding table, `.node-box.status-*` class for stroke color only.
- [x] 5.5 GREEN edit `webview/styles.css`: change the four `.node-box.status-*` rules from `fill:` to `stroke:`; add `.node-box { fill: none }` and `.node-box.status-unchanged { stroke: #6e7681 }`.

## Phase 6: Slice B — Directional Edges and Ambiguity

- [x] 6.1 RED new case: arrowhead — resolved `import`/`call` edges carry `marker-end`; ambiguous/unresolved edges do not.
- [x] 6.2 RED new case: `contains` contributes no `<g class="edge">` element, while surviving edges retain their original `graph.edges` array indices.
- [x] 6.3 RED rewrite `graphView.test.ts` L49-54 (`"explicitly marks ambiguous and unresolved edges…"`): keep `data-resolution` assertions; drop reliance on `<text class="edge-label">`; add "no `marker-end` on ambiguous/unresolved".
- [x] 6.4 RED delete `graphView.test.ts` L81-85 (`"lays relationship text below every node box"`) — `.edge-label` text no longer exists; covered by 6.1/6.3 instead.
- [x] 6.5 GREEN implement one `<defs>` block with `#acm-arrow-import`/`#acm-arrow-call` markers; `renderEdge` computing anchors (source bottom-center, target top-center) and elbow path (`ELBOW_DROP = 16`); ambiguous/unresolved/target-not-in-view edges render a dashed downward stub (`STUB_LEN = 28`, `stroke-dasharray="4 3"`, no `marker-end`), retaining `<title>` and `data-resolution`; `.edge-import`/`.arrow-import` `#4f9cf9`, `.edge-call`/`.arrow-call` `#c586c0`, ambiguity `#f0883e`; edge loop iterates `graph.edges` by index so `contains` skips rendering without shifting indices.

## Phase 7: Slice B — Flat-Degradation Threshold

- [x] 7.1 RED new case: degradation — a 61-node graph renders via the flat stack yet stays outline-only with drawn edges (kind encoding and markers preserved).
- [x] 7.2 GREEN add `export const NESTED_LAYOUT_LIMITS = { nodes: 60, edges: 120 } as const` in `webview/graphView.ts`; check at the top of `renderGraphSvg`, returning `renderFlatSvg(...)` above the limit; `renderFlatSvg` keeps `y = 24 + index * 48` stack geometry but shares `renderNodeRect`/`renderEdge` with the nested path.
- [x] 7.3 REFACTOR: confirm `NESTED_LAYOUT_LIMITS` reads/writes nothing shared with `OVERSIZED_THRESHOLDS` (300/600) and the two gates stay sequential and independent.

## Phase 8: Data Attribute Contract and Full Verification

- [x] 8.1 RED new case: `data-*` stability — full attribute set (`data-node-id`, `data-node-kind`, `data-change-status`, `data-edge-index`, `data-edge-kind`, `data-resolution`) present with exact current values under the new nested/styled rendering.
- [x] 8.2 GREEN verify `webview/index.ts`'s `querySelectorAll("[data-node-id]")`/`("[data-edge-index]")` wiring and `graph.edges[index]`/`message.edgeSources[index]` lookups require zero edits; if any edit is needed, treat it as a contract break and stop.
- [x] 8.3 Verify `test/e2e/scenarios.ts` diagram and navigation scenarios pass unchanged — the top-layer click-contract proof.
- [x] 8.4 Run `npm run lint && npm run typecheck && npm run test` (unit + integration + e2e) and confirm all green; document any environment-gated skips per existing README convention.
