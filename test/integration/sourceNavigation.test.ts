...
describe("source navigation", () => {
  ...
  it("correlates a changed function between two states with left and right spans", () => {
    ...
    const left: CapturedState = { snapshot: leftSnapshot, files: [{ path: "m.py", content: leftContent, provenance: "tracked" }] };
    const right: CapturedState = { snapshot: rightSnapshot, files: [{ path: "m.py", content: rightContent, provenance: "tracked" }] };
