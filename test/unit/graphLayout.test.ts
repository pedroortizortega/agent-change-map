...
/**
 * Permanent performance-regression guard for the FINAL, landed visibility-graph + A* router
 * (PR1-PR4 of `edge-router-performance`, all complete — see apply-progress.md). Historically this
 * describe block held only a stale `{60,120}` probe written against the OLD O(N^3)-ish
 * candidate-enumeration router (see apply-progress.md's Phase-0 gate and PR0/PR3a/crossing-fix
 * sections for that router's full measured history); that router no longer exists in production
 * (`webview/edgeGeometry.ts`'s `edgePathsFor` now delegates to `webview/routingGraph.ts` +
 * `webview/routeSearch.ts`), so this block now locks in the two ms-budget requirements from
 * `openspec/changes/edge-router-performance/specs/change-map-visualization/spec.md` against the
 * CURRENT implementation, with a generous catastrophic-regression-guard margin rather than a tight
 * budget-matching assertion (see each test's own comment for its specific margin reasoning). A
 * third budget — the 500ms scoped drag-commit re-route bound — is already covered by the
 * `routedPaths — PR4 scoped drag re-route` describe block's own
 * "completes a drag-commit re-route comfortably under 500ms..." test above; not duplicated here.
 *
 * `openspec/changes/edge-router-performance/perf/measure-layout.ts` remains as a standing,
 * manually-run diagnostic script (documented there) for exploring the full size curve across many
 * points; these two committed tests are the permanent CI-enforced regression guards for the two
 * spec-mandated boundary points specifically.
 */
describe("performance probe near the OVERSIZED_THRESHOLDS boundary (design.md, unmeasured risk)", () => {
  ...
  it("computes layoutGraph at {nodes:60, edges:120} (NESTED_LAYOUT_LIMITS' own boundary) within its 2000ms spec budget", () => {
    ...
    // Locks in the spec's own NESTED_LAYOUT_LIMITS boundary budget (see
    // specs/change-map-visualization/spec.md, "Nested layout threshold reflects a measured render
    // budget"): must complete under 2000ms. Real measured cost on this machine with the current
    // (final) router is ~12ms — this bound keeps a wide (~166x) catastrophic-regression margin
    // while still matching the exact spec number, not an arbitrarily looser one.
    expect(elapsedMs).toBeLessThan(2_000);
  });

  it("computes layoutGraph at {nodes:300, edges:600} (OVERSIZED_THRESHOLDS boundary) within its 10000ms spec budget", () => {
    const graph = flatGraph(300, 600);
    const start = performance.now();
    const result = layoutGraph({ graph, diff: [], untrackedPaths: [], overrides: new Map() });
    const elapsedMs = performance.now() - start;
    ...
    console.log(`[perf-probe] layoutGraph({nodes:300, edges:600}) took ${elapsedMs.toFixed(2)}ms`);
    // Locks in the spec's own OVERSIZED_THRESHOLDS boundary budget (see
    // specs/change-map-visualization/spec.md, "Oversized-graph gate threshold reflects a measured
    // render budget"): must complete under 10000ms. This is intentionally the spec's exact hard
    // budget, not a tight ~5000ms assertion that could flake on a slower CI machine — real measured
    // cost across this change's own history (PR0 gate, CROSSING_BASE sweep, crossing-fix
    // re-measurement; see apply-progress.md) clusters at ~4.2s-6.4s on this machine, so 10000ms
    // still keeps real (if narrower than the 60/120 case) margin while catching a genuine
    // catastrophic regression — e.g. an accidental reintroduction of the old O(N^3)-ish
    // candidate-enumeration router this change replaced.
